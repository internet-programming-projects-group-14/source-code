...
const getNetworkInfo = async () => {
  ...
  try {
    ...
    console.log(`Using data from ${service}:`, data)
    ...
    console.log(" Final network info:", networkInfo)
    return networkInfo
  } catch (error) {
...
const cleanISPName = (rawIsp) => {
  ...
  console.log("🧹 Cleaning ISP name:", rawIsp)
  ...
  for (const [key, value] of Object.entries(ispMappings)) {
    if (cleaned.toUpperCase().includes(key)) {
      console.log(` Matched ISP: ${key} -> ${value}`)
      return value
    }
  }

  console.log(" Cleaned ISP name:", cleaned)
  return cleaned
}
...
const performDownloadTest = async (onProgress) => {
  ...
  try {
    ...
    const speedMbps = (arrayBuffer.byteLength * 8) / (durationSeconds * 1024 * 1024)
...
const saveTestResult = async (result) => {
  try {
    const historyItem = { id: Date.now().toString(), timestamp: Date.now(), ...result }
...
export default function DebugNetworkInfo() {
  ...
  const initializeApp = async () => {
    ...
    try {
      const [netInfo, historyData] = await Promise.all([getNetworkInfo(), loadTestHistory()])

      console.log("📡 Setting network info:", netInfo)
  ...
  const refreshNetworkInfo = async () => {
    ...
    try {
      ...
      console.log(" Network info refreshed")
    } catch (error) {
  ...
  const runSpeedTest = async () => {
    ...
    const startTime = Date.now()
    const finalResults = { downloadSpeed: 0, uploadSpeed: 0, latency: 0, jitter: 0, packetLoss: 0 }

    try {
      // Latency Test
      setCurrentPhase("latency")
      const latencyResult = await performLatencyTest((progress) => setProgress(progress))
      finalResults.latency = latencyResult.latency
      finalResults.jitter = latencyResult.jitter
      setResults((prev) => ({ ...prev, latency: latencyResult.latency, jitter: latencyResult.jitter }))
      ...
      setResults((prev) => ({ ...prev, downloadSpeed: downloadResult.speedMbps }))
      ...
      setResults((prev) => ({ ...prev, uploadSpeed: uploadResult.speedMbps, packetLoss: finalResults.packetLoss }))
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        <Card>
          <View style={styles.cardHeader}>
            ...
            <TouchableOpacity onPress={refreshNetworkInfo} style={styles.refreshButton} disabled={isLoadingNetwork}>
              <RefreshCw color="#ffffff" size={16} style={isLoadingNetwork ? { opacity: 0.5 } : {}} />
            </TouchableOpacity>
          </View>
          ...
          {testState === "idle" && (
            <View style={styles.testInterface}>
              <View style={styles.idleContainer}>
                <LinearGradient colors={["#10b98133", "#3b82f633"]} style={styles.idleIcon}>
                  <Wifi color="#ffffff" size={64} />
                </LinearGradient>
                <View style={styles.idleText}>
                  <Text style={styles.idleTitle}>Ready to Test</Text>
                  <Text style={styles.idleSubtitle}>Measure your network performance with reliable testing</Text>
                </View>
                ...
                <View style={styles.progressContainer}>
                  ...
                  <View style={styles.progressContent}>
                    ...
                    <Text style={styles.progressValue}>
                      {currentPhase === "latency" ? `${progress.toFixed(0)}%` : `${realtimeSpeed.toFixed(1)}`}
                    </Text>
                    <Text style={styles.progressUnit}>{currentPhase === "latency" ? "Progress" : "Mbps"}</Text>
                  </View>
                </View>
                <View style={styles.runningText}>
                  <Text style={styles.runningTitle}>{getPhaseLabel()}</Text>
                  <Text style={styles.runningSubtitle}>Please wait while we analyze your connection</Text>
                </View>
              </View>
            </View>
          )}
          ...
          <View style={styles.cardContent}>
            {history.length === 0 ? (
              <View style={styles.emptyHistory}>
                <Text style={styles.emptyHistoryText}>No test history available</Text>
                <Text style={styles.emptyHistorySubtext}>Run a speed test to see your results here</Text>
              </View>
            ) : (
                  ...
                  <View key={item.id} style={styles.historyItem}>
                    ...
                    <View style={styles.historyResults}>
                      <Text style={styles.historySpeed}>
                        {item.downloadSpeed.toFixed(1)} / {item.uploadSpeed.toFixed(1)} Mbps
                      </Text>
                      <Text style={styles.historyLatency}>{item.latency.toFixed(0)}ms latency</Text>
                    </View>
                  </View>
                ))}
              </View>
...
const styles = StyleSheet.create({
  ...
  cardTitle: { fontSize: 16, fontWeight: "600", color: "#ffffff", marginLeft: 8 },
  ...
  infoLabel: { fontSize: 14, color: "rgba(255, 255, 255, 0.6)", marginBottom: 4 },
  ...
  idleTitle: { fontSize: 20, fontWeight: "600", color: "#ffffff", marginBottom: 8 },
  idleSubtitle: { fontSize: 16, color: "rgba(255, 255, 255, 0.7)", textAlign: "center" },
  startButton: {
  ...
  startButtonText: { color: "#ffffff", fontSize: 16, fontWeight: "600", marginLeft: 8 },
  runningContainer: { alignItems: "center" },
  progressContainer: { position: "relative", alignItems: "center", justifyContent: "center", marginBottom: 24 },
  ...
  progressValue: { fontSize: 24, fontWeight: "bold", color: "#ffffff", marginTop: 8 },
  progressUnit: { fontSize: 12, color: "rgba(255, 255, 255, 0.6)" },
  runningText: { alignItems: "center" },
  runningTitle: { fontSize: 18, fontWeight: "600", color: "#ffffff", marginBottom: 4 },
  runningSubtitle: { fontSize: 16, color: "rgba(255, 255, 255, 0.7)", textAlign: "center" },
  completedContainer: { alignItems: "center" },
  completedHeader: { alignItems: "center", marginBottom: 24 },
  completedTitle: { fontSize: 20, fontWeight: "600", color: "#ffffff", marginBottom: 8 },
  ...
  resultValue: { fontSize: 24, fontWeight: "bold", color: "#ffffff", marginTop: 8 },
  resultLabel: { fontSize: 14, color: "rgba(255, 255, 255, 0.6)", marginTop: 4, marginBottom: 8 },
  metricsGrid: { flexDirection: "row", gap: 16, marginBottom: 24, width: "100%" },
  metricItem: { flex: 1, alignItems: "center" },
  metricValue: { fontSize: 18, fontWeight: "600", color: "#ffffff", marginTop: 4 },
  metricLabel: { fontSize: 12, color: "rgba(255, 255, 255, 0.6)", marginTop: 2 },
  ...
  actionButtonText: { color: "#ffffff", fontSize: 16, fontWeight: "500", marginLeft: 8 },
  clearButton: {
  ...
  emptyHistoryText: { fontSize: 16, fontWeight: "500", color: "rgba(255, 255, 255, 0.7)", marginBottom: 4 },
  emptyHistorySubtext: { fontSize: 14, color: "rgba(255, 255, 255, 0.5)", textAlign: "center" },
  historyItems: { gap: 12 },
  historyItem: {
    flexDirection: "row",
    justifyContent: "space-between",
  ...
  historyLocation: { fontSize: 14, color: "rgba(255, 255, 255, 0.6)", marginTop: 2 },
  historyResults: { alignItems: "flex-end" },
  historySpeed: { fontSize: 16, fontWeight: "500", color: "#ffffff" },
  historyLatency: { fontSize: 14, color: "rgba(255, 255, 255, 0.6)", marginTop: 2 },
})